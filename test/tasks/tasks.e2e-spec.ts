...
describe('Tasks - /tasks (e2e)', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      imports: [
        TypeOrmModule.forRoot({
          type: 'postgres',
          host: 'localhost',
          port: 5431, // テスト用コンテナのポートを指定
          username: 'root',
          password: 'secret',
          database: 'mydb-test', // テスト用のデータベース名を指定
  ...
  it('Get all tasks [GET /tasks]', async () => {
    const tasksArray = await tasksRepository.save([taskA, taskB, taskC]);

    return request(app.getHttpServer())
      .get('/tasks')
      .expect(200)
      .then(({ body }) => {
        expect(body).toEqual(tasksArray);
      });
  });
